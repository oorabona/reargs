  ...
  __checkArg(arg, askforId, currentParam, allowStartingIndex = 0) {
    for (const pType of ['short', 'long']) {
      ...
      if (currentParam[pType] instanceof RegExp) {
        const matchGroups = currentParam[pType].exec(arg)

        this.debug(`arg='${arg}' pType='${pType}' re='${currentParam[pType].toString()}' matchGroups='${matchGroups}'`)

        if (matchGroups !== null && matchGroups.index <= allowStartingIndex) {
          this.askfor[askforId].result = matchGroups

          if (matchGroups.groups) {
            let defaultValues = {}
            if (isObject(this.askfor[askforId].values)) {
              defaultValues = this.askfor[askforId].values
            }
            this.askfor[askforId].values = {}
            for (const [key, value] of Object.entries(matchGroups.groups)) {
              const defValue = value || defaultValues[key]
              this.askfor[askforId].values[key] = defValue
            }
          } else {
    ...
    for (const askforId in this.params) {
      ...
      if (currentParam.stopParse) {
        ...
        if (hasMatched) {
          ...
          if (index < minIndex) {
            minIndex = index
            lengthOfStopParameter = this.askfor[askforId].result[0].length
