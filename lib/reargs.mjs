...
export default class Reargs {
  ...
  constructor(params = {}, opts = null, debug = false) {
    ...
    nunjucks.configure({
      autoescape: this.opts.nunjucksAutoEscape
    })
  ...
  set(id, param) {
    ...
    // check for accepted types first
    for (const pType of ['short', 'long']) {
      if (types[pType] !== 'undefined' && types[pType] !== 'string' && !(_param[pType] instanceof RegExp)) {
    ...
    for (const pType of ['short', 'long']) {
      if (types[pType] !== 'undefined') {
        const pCurrent = !_param[pType].source ? _param[pType] : _param[pType].source

        if (typeof(_param.captureMultiple) === 'string') {
          _param.captureMultiple = new RegExp(_param.captureMultiple)
        }

        const pCurrentZ = pCurrent.split(' ').join('\x00')
        if (_param.captureMultiple instanceof RegExp) {
          _param[pType] = new RegExp(`\x00*${pCurrentZ}(\x00|$)?`)
        } else {
          _param[pType] = new RegExp(`\x00*${pCurrentZ}(\x00|$)`)
        }
